...
async def handle_message(update: Update, context: CallbackContext) -> None:
    ...
    if user_data is None:
        cursor.execute("INSERT INTO users (user_id, name, location, status) VALUES (?, ?, ?, ?)", (user_id, text, "", "waiting_for_location"))
        conn.commit()
        await update.message.reply_text(f"✅ اسمت ثبت شد: {text}\nحالا می‌پرسم که آیا در دفتر آبادان هستی؟\nلطفاً یکی از دکمه‌های زیر رو انتخاب کن.",
        ...
        if status == "waiting_for_payment":
            await add_payment(update, context, name, location)  # 📌 مقدار `location` را پاس بده
        else:
...
async def button(update: Update, context: CallbackContext) -> None:
    ...
    if choice == 'bale':
        cursor.execute("UPDATE users SET location = ?, status = ? WHERE user_id = ?", ("آبادان", "waiting_for_payment", user_id))
        conn.commit()
        await query.answer("📍 موقعیت شما به آبادان تغییر یافت.")
    elif choice == 'na':
        cursor.execute("UPDATE users SET location = ?, status = ? WHERE user_id = ?", ("", "waiting_for_payment", user_id))
        conn.commit()
